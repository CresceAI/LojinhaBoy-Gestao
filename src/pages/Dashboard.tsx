...
const Dashboard = () => {
  ...
  // --- LÓGICA DE INSIGHTS (Proteção contra NaN e Null) ---
  ...
  return (
    /* A KEY ABAIXO É A CURA PARA O ERRO REMOVECHILD NO MOBILE */
    <div key="dashboard-main-view" className="space-y-6 animate-fade-in pb-10">
      ...
      {/* CARD PRINCIPAL - CORES CORRIGIDAS PARA LIGHT MODE */}
      <div className="balance-card bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-6 rounded-[2rem] shadow-sm border border-primary/10 relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        />
        <StatSmall label="Lucro a Receber" value={formatCurrency(insights.lucroProjetado - insights.lucroRealizado)} icon={PieChart} color="text-primary" />
        {!isMobile && (
          <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="font-bold flex items-center gap-2"><Clock className="w-4 h-4 text-warning" /> Próximos Recebimentos</h3>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="font-bold flex items-center gap-2"><TrendingUp className="w-4 h-4 text-success" /> Ganhos Recentes</h3>
          </div>
          <div className="space-y-3">
...
// --- SUB-COMPONENTES (COM TYPESCRIPT CORRIGIDO) ---
const QuickAction = ({ icon: Icon, label, color, onClick }: any) => (
  <button onClick={onClick} className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-card border border-border/40 hover:bg-secondary transition-all active:scale-95">
    <div className={`p-3 rounded-xl ${color}`}><Icon className="w-5 h-5" /></div>
    <span className="text-[10px] font-bold uppercase tracking-tighter">{label}</span>
  </button>
);

const StatSmall = ({ label, value, icon: Icon, color, trend }: { label: string; value: string | number; icon: any; color: string; trend?: string; }) => (
...
const TransactionItem = ({ title, subtitle, value, isVencido, type = "neutral" }: { title: string; subtitle: string; value: string; isVencido?: boolean; type?: "positive" | "neutral"; }) => (
  <div className="flex items-center justify-between p-4 bg-card rounded-2xl border border-border/30 hover:border-primary/20 transition-all">
    <div className="flex items-center gap-3">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${type === 'positive' ? 'bg-success/10' : isVencido ? 'bg-destructive/10' : 'bg-primary/10'}`}>
      ...
      <div>
        ...
        <p className={`text-[11px] ${isVencido ? 'text-destructive font-bold' : 'text-muted-foreground'}`}>{isVencido ? 'EM ATRASO' : subtitle}</p>
      </div>
    </div>
    <p className={`text-sm font-bold ${type === 'positive' ? 'text-success' : 'text-foreground'}`}>{value}</p>
  </div>
);
