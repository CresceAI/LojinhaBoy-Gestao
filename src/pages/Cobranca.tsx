...
const CobrancaPage = () => {
  ...
  const emprestimosAbertos = emprestimos
  ...
  const emprestimosFiltrados = emprestimosAbertos.filter(emp => {
    const nomeCliente = getClienteNome(emp.cliente_id).toLowerCase();
    ...
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="space-y-3">
        {emprestimosFiltrados.map((emp) => {
          const isVencido = new Date(emp.data_vencimento) < new Date();
          const diasAtraso = calcularDiasAtraso(emp.data_vencimento);
          const valorDevendo = Number(emp.valor_total) - Number(emp.valor_pago);
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-3 flex-1">
                    <div className="flex items-center gap-3">
                      <h3 className="text-xl font-bold">{getClienteNome(emp.cliente_id)}</h3>
                      {isVencido && (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      ...
                      <div className="p-3 bg-muted/50 rounded-lg">
                        ...
                        <p className="text-lg font-semibold text-success">{formatCurrency(Number(emp.juros))}</p>
                      </div>
                      ...
                      <div className="p-3 bg-muted/50 rounded-lg">
                        ...
                        <p className={`text-sm font-medium ${isVencido ? 'text-destructive' : 'text-foreground'}`}>
                          {formatDate(emp.data_vencimento)}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <span>Emprestado: {formatCurrency(Number(emp.valor))}</span>
                      <span>•</span>
                      <span>Início: {formatDate(emp.data_inicio)}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
      ...
      {emprestimosFiltrados.length === 0 && emprestimosAbertos.length > 0 && (
