...
const RelatoriosAvancados = () => {
  ...
  const loadData = () => {
    ...
    const totalEmprestado = data.reduce((acc, emp) => acc + emp.valorTotal, 0);
    const totalRecebido = data.reduce((acc, emp) => acc + emp.valorPago, 0);
    const totalAberto = data.filter(e => e.status === 'ativo').reduce((acc, emp) => acc + (emp.valorTotal - emp.valorPago), 0);
    const totalVencido = data.filter(e => e.status === 'vencido').reduce((acc, emp) => acc + emp.valorTotal, 0);
        ...
        <CardContent>
          <div className="space-y-4">
            {['ativo', 'pago', 'vencido'].map((status) => {
              const count = emprestimos.filter(e => e.status === status).length;
